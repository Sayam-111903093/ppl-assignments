class Animal(object):
	...
	def introduce(self):
		print('\nMy name is', self.name, 'and I am a', self.species(), '\nMy colour is', self.colour, '\nI say', self.sound(), '\nI live', self.home(), '\nI eat', self.food())
...
class snow_animals(Animal):

	def home(self):
		return 'in snows and polar areas.'
...
class omnivorous(Animal):
	...
	def food(self):
		return 'chapati, milk, meat, etc.'

class snowanimals(Animal):
...
class bear(wild_animal, omnivorous):
...
class polarbear(snow_animals, carnivorous):
